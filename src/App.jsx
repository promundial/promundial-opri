...
async function generateOPRIReport(eng, allResponses, CORE_DIMS, FULL_DIMS, DEEP_MODULES, computeOPRI, computeDeep, checkL2, checkL3) {
  ...
  // We set the loading state via direct DOM mutation (body.innerHTML), not
  // document.write(). The window opened by window.open("", ...) already has
  // a valid empty document (like about:blank) — no need to write/open/close.
  win.document.title = "OPRI™ · Generando reporte…";
  ...
  // Write via direct DOM mutation (head.innerHTML / body.innerHTML) instead
  // of document.write() or a Blob-URL navigation. Both of those trigger
  // separate, well-documented Safari bugs when combined with window.print():
  // document.write() a second time on an already-loaded document can blank
  // the tab after printing, and blob: URLs can fail to resolve inside
  // Safari's internal print pipeline (which runs in a different process than
  // the tab's JS heap where the Blob lives). Mutating the DOM of the window
  // we already have open sidesteps both — nothing is re-navigated or
  // re-parsed via document.write, so print behaves like it would on any
  // normal, network-loaded page.
  win.document.title = "OPRI™ Report — " + eng.company;
